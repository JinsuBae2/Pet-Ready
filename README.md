...
# 🐶 펫-레디 (Pet-Ready)
> **IoT 인형을 활용한 반려견 양육 시뮬레이터 프로젝트**

실제 반려견 양육 시 마주하는 고충(새벽 짖음, 규칙적인 식사, 산책 등)을 IoT 기술로 구현하여 사용자의 양육 적합도를 점수화하는 서비스입니다.

---

## 📂 1. 프로젝트 통합 디렉터리 구조 (Monorepo)
...
## 🌿 2. 깃허브 협업 및 브랜치 전략
본 프로젝트는 안정적인 코드 통합을 위해 **Develop 브랜치 전략**을 사용합니다.

### **브랜치 규칙**
* **`main`**: 최종 발표 및 시연용 (Phase 6 최종 통합 시 사용).
* **`develop`**: 모든 팀원의 코드가 합쳐지는 메인 개발 브랜치.
* **개별 브랜치**: 기능 개발 시 `develop`에서 분기하여 사용합니다.
    * 형식: `feat/{파트}/{기능명}` (예: `feat/be/login`, `feat/aos/gps`, `feat/hw/sensor`)

---

## 🛠 3. 디렉터리 통합 및 작업 프로세스 (중요)
